...
# Install MITE-Hunter
RUN wget http://target.iplantcollaborative.org/mite_hunter/MITE%20Hunter-11-2011.zip && \
	unzip MITE*.zip && \
	rm *.zip && \
	mv MITE* mitehunter \
	&& cd mitehunter \
	&& perl MITE_Hunter_Installer.pl -d /usr/local/mitehunter/ -f /usr/bin/formatdb -b /usr/bin/blastall_old -m /usr/local/mdust/mdust -M /usr/bin/muscle
